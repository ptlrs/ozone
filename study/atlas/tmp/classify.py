#!/usr/bin/env python3
"""Deterministic classifier over raw_classes.tsv.

Reads each Java file to derive: component, feature, kind, logic_weight,
concurrency (heuristic), persistence (heuristic), role_one_liner (first
Javadoc sentence), key_collaborators (naive: 'new X(' / 'X.class' inside),
test_exemplar (name match, integration-preferred), difficulty/study_minutes
(from loc_code+kind), and package-derived hints.

Writes:
  study/atlas/tmp/classes.jsonl  (one JSON per class)
  study/atlas/tmp/features.tsv   (component<TAB>feature<TAB>count)
  study/atlas/tmp/anchors.tsv    (classes flagged as anchors for deep pass)
"""
...
# ---- component + feature routing rules (path-based, first match wins) ----
# Each rule: (path substring, component, feature)
RULES = [
    # ---- RocksDB primitives ----
    ...
    # ---- SCM ----
    ('hadoop-hdds/server-scm/',            'SCM',       ''),   # feature filled by sub-package
    ...
    # ---- HddsCommon / Framework split by sub-package ----
    ('hadoop-hdds/framework/',             'Framework', ''),   # split below
    ('hadoop-hdds/common/',                'HddsCommon',''),   # split below
...
# --- feature sub-routing for the big modules (path substring or package fragment -> feature) ---
FEATURE_RULES = {
  'SCM': [
  ...
  'DN': [
    ...
    ('/keyvalue/statemachine/',            'ratis-statemachine'),
    ('/keyvalue/',                         'kv-container'),
    ('/transport/server/ratis/',           'ratis-statemachine'),
    ('/transport/server/',                 'grpc-server'),
  ...
  'Framework': [
    # split into Ratis-integration / Security / HddsCommon per user decision #1
  ...
  'HddsCommon': [
    ...
    ('/scm/ha/',                           'ratis-integration'),   # override to Ratis-integration
...
TABLE_RE = re.compile(r'(?:getTable|"(?:volumeTable|bucketTable|keyTable|fileTable|dirTable|deletedTable|deletedDirTable|multipartInfoTable|s3SecretTable|prefixTable|snapshotInfoTable|snapshotRenamedTable|openKeyTable|openFileTable|delegationTokenTable|principalToAccessIdsTable|tenantAccessIdTable|tenantStateTable|meta|Table)")')
    ...
    if comp in FEATURE_RULES:
        for tup in FEATURE_RULES[comp]:
            if len(tup) == 2:
                ...
                # Framework split rule: (sub, feature, comp_override)
                sub, f, comp_over = tup
                if sub in path:
                    feat = f
                    comp = comp_over
...
def kind_of(fqcn: str, path: str, body: str) -> str:
    b = body[:8000]
    ...
    if re.search(r'@Path\(|@Provider|@Endpoint', b): return 'service'
    ...
    if 'DTO' in fqcn or 'Info' in fqcn.rsplit('.',1)[-1] or 'Response' in fqcn.rsplit('.',1)[-1] or 'Request' in fqcn.rsplit('.',1)[-1]:
        # Info/Request/Response often DTO but sometimes coordinator; refine by loc_code
        return 'dto'
    return 'service'

def logic_weight_of(kind: str, loc_code: int) -> str:
    if kind in ('data','dto','exception','config','rpc-stub'):
        return 'data-only'
    if loc_code < 60:
        return 'mixed'
    if loc_code >= 200:
...
def concurrency_of(body: str) -> str:
    for rx, tag in CONCURRENCY_MARKERS:
        if rx.search(body):
            return tag
    return 'single-threaded'

def persistence_of(body: str) -> str:
    m = re.findall(r'(?:get|open)Table\("(\w+)"', body)
    if m:
        return 'RocksDB:'+','.join(sorted(set(m))[:4])
    if 'RocksDB' in body or 'ManagedRocksDB' in body:
        return 'RocksDB'
    if re.search(r'\.dat\b|writeToDisk|FileOutputStream|Files\.write', body):
        return 'on-disk'
    return 'in-memory'
...
def role_of(body: str, cls: str) -> str:
    # find first javadoc block preceding a class/interface/enum decl
...
with open(RAW) as f:
    for line in f:
        ...
        # difficulty & study minutes
        if kind in ('data','dto','exception'):
            diff, mins = 1, 10
        elif kind in ('config','rpc-stub','util','factory','metrics','cli','interface'):
            diff, mins = 2, 20
...
# --- anchors: top ~20% by loc_code within each (component,feature), min 300 LOC, plus state-machines and coordinators ---
by_group = {}
for r in rows:
    by_group.setdefault((r['component'], r['feature']), []).append(r)
anchors = []
for grp, items in by_group.items():
    ...
    for r in items[:cutoff]:
        if int(r['loc_code'].rstrip('~')) >= 200 or r['kind'] == 'state-machine':
            anchors.append(r)
    for r in items:
        if r['kind'] in ('state-machine',) and r not in anchors:
            anchors.append(r)
