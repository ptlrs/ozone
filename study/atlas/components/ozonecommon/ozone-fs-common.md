...
|--:|---|---|---|--:|--:|---|
| 2199 | `org.apache.hadoop.fs.ozone.OzoneTrashPolicy` | service | mixed | 125~ | 30 | TrashPolicy for Ozone Specific Trash Operations. |
...
- HDDS-11749. Extract moveToTrash implementation to client code (created `OzoneTrashPolicy`)
- HDDS-12789. Support move expired key to trash if trash is enabled
- HDDS-12270. Fix license headers and imports for ozone-common

## Sharp edges

- `OzoneTrashPolicy` constructs the trash path using the current UGI username, but HDDS-14177 noted that OFS should use the internal FS username instead. Callers in the S3 Gateway may have a different UGI than expected, leading to trash files in an unexpected location.
...
2. Why was `OzoneTrashPolicy` extracted from `OzoneFileSystem` in HDDS-11749?
