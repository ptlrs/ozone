...
|--:|---|---|---|--:|--:|---|
| 898 | `org.apache.hadoop.ozone.snapshot.SnapshotDiffReportOzone` | service | mixed | 125~ | 30 | Snapshot diff report. |
| 899 | `org.apache.hadoop.ozone.snapshot.SnapshotDiffResponse` | dto | data-only | 150~ | 10 | POJO for Snapshot Diff Response. |
| 900 | `org.apache.hadoop.ozone.snapshot.ListSnapshotDiffJobResponse` | dto | data-only | 25~ | 10 | POJO for list snapshot diff job API. |
| 901 | `org.apache.hadoop.ozone.snapshot.ListSnapshotResponse` | dto | data-only | 25~ | 10 | POJO for list snapshot info API. |
| 902 | `org.apache.hadoop.ozone.snapshot.CancelSnapshotDiffResponse` | dto | data-only | 25~ | 10 | POJO for Cancel Snapshot Diff Response. |
| 903 | `org.apache.hadoop.ozone.snapshot.SubmitSnapshotDiffResponse` | dto | data-only | 25~ | 10 | POJO for Submit Snapshot Diff Response. |
...
1. Why was the snapshot diff API split into submit/get RPCs (HDDS-14829), and which response classes correspond to each operation?
2. What does `SnapshotDiffReportOzone.getToken()` return when there are no more pages, and what bug was fixed by HDDS-12823?
...
Answer 1: The original single-call diff API would block indefinitely for large diffs. HDDS-14829 split it into `submitSnapshotDiff` (returns `SubmitSnapshotDiffResponse` with a job ID) and `getSnapshotDiffReport` (returns `SnapshotDiffResponse` with status and paginated results). `CancelSnapshotDiffResponse` corresponds to the cancel operation.
Answer 2: `getToken()` returns null or empty string when there are no more pages. HDDS-12823 fixed a bug in `fromProtobuf` where an empty proto `bytes` field was deserialized as an empty `ByteString` and then converted to an empty string rather than null, causing callers to think there was a non-null pagination token and make an extra (empty) page request.
