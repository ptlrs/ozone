...
|--:|---|---|---|--:|--:|---|
| 2404 | `org.apache.hadoop.ozone.recon.spi.ReconContainerMetadataManager` | interface | mixed | 50~ | 20 | The Recon Container DB Service interface. |
| 2405 | `org.apache.hadoop.ozone.recon.spi.ReconGlobalStatsManager` | interface | mixed | 25~ | 20 | The Recon Global Stats DB Service interface. |
| 2406 | `org.apache.hadoop.ozone.recon.spi.StorageContainerServiceProvider` | interface | mixed | 25~ | 20 | Interface to access SCM endpoints. |
| 2407 | `org.apache.hadoop.ozone.recon.spi.OzoneManagerServiceProvider` | interface | mixed | 25~ | 20 | Interface to access OM endpoints. |
| 2408 | `org.apache.hadoop.ozone.recon.spi.ReconNamespaceSummaryManager` | interface | mixed | 25~ | 20 | Interface for DB operations on NSSummary. |
| 2409 | `org.apache.hadoop.ozone.recon.spi.HddsDatanodeServiceProvider` | interface | mixed | 25~ | 20 | Interface to access datanode endpoints. |
| 2410 | `org.apache.hadoop.ozone.recon.spi.ReconFileMetadataManager` | interface | mixed | 25~ | 20 | The Recon File Metadata DB Service interface for file size counts. |
| 2411 | `org.apache.hadoop.ozone.recon.spi.MetricsServiceProvider` | metrics | mixed | 25~ | 20 | Interface to access Ozone metrics. |

### Sub-feature: `spi.impl`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 2412 | `org.apache.hadoop.ozone.recon.spi.impl.ReconContainerMetadataManagerImpl` | service | logic-heavy | 400~ | 60 | Implementation of the Recon Container DB Service. |
| 2413 | `org.apache.hadoop.ozone.recon.spi.impl.StorageContainerServiceProviderImpl` | service | mixed | 125~ | 30 | Implementation for StorageContainerServiceProvider that talks with actual cluster SCM. |
| 2414 | `org.apache.hadoop.ozone.recon.spi.impl.ReconRDBSnapshotProvider` | service | mixed | 125~ | 30 | n OM follower uses: a chunked POST /v2/dbCheckpoint request that carries a toExcludeList[] of the parts already recei... |
| 2415 | `org.apache.hadoop.ozone.recon.spi.impl.ReconDBProvider` | service | mixed | 100~ | 30 | Provider for Recon's RDB. |
| 2416 | `org.apache.hadoop.ozone.recon.spi.impl.PrometheusServiceProviderImpl` | service | mixed | 100~ | 30 | Implementation of the Prometheus Metrics Service provider. |
| 2417 | `org.apache.hadoop.ozone.recon.spi.impl.ReconFileMetadataManagerImpl` | service | mixed | 75~ | 30 | Implementation of the Recon File Metadata DB Service. |
| 2418 | `org.apache.hadoop.ozone.recon.spi.impl.ReconDBDefinition` | service | mixed | 75~ | 30 | RocksDB definition for the DB internal to Recon. |
| 2419 | `org.apache.hadoop.ozone.recon.spi.impl.JmxServiceProviderImpl` | service | mixed | 50~ | 30 | Implementation of the Jmx Metrics Service provider. |
| 2420 | `org.apache.hadoop.ozone.recon.spi.impl.ReconNamespaceSummaryManagerImpl` | service | mixed | 50~ | 30 | Wrapper functions for DB operations on recon namespace summary metadata. |
| 2421 | `org.apache.hadoop.ozone.recon.spi.impl.ReconGlobalStatsManagerImpl` | service | mixed | 50~ | 30 | Implementation of the Recon Global Stats DB Service. |
| 2422 | `org.apache.hadoop.ozone.recon.spi.impl.KeyPrefixContainerCodec` | util | mixed | 125~ | 20 | Codec to serialize/deserialize KeyPrefixContainer. |
| 2423 | `org.apache.hadoop.ozone.recon.spi.impl.ContainerKeyPrefixCodec` | util | mixed | 50~ | 20 | Codec to serialize/deserialize ContainerKeyPrefix. |
| 2424 | `org.apache.hadoop.ozone.recon.spi.impl.OzoneManagerServiceProviderImpl` | service | logic-heavy | 675~ | 10 | Implementation of the OzoneManager Service provider. |
...
- HDDS-13792. Move container related metadata from Derby to OM DB (reverted in HDDS-13808)
- HDDS-13956. Code reliability and data integrity improvements in Recon
...
- `ReconContainerMetadataManagerImpl` maintains `CONTAINER_KEY` and `KEY_CONTAINER` as two independent tables that must stay in sync. Any failure mid-batch-write that leaves one table updated and the other not will corrupt the bidirectional index. There is no compensating read-repair. (HDDS-13956 hardened some of these paths.)
- `ReconRDBSnapshotProvider` supports resumable tarball downloads via a `toExcludeList` of already-received parts. If Recon crashes mid-download and the partial state is not cleaned up, the next download attempt may incorrectly skip parts. HDDS-15766 improved reliability here.
