...
|--:|---|---|---|--:|--:|---|
| 2380 | `org.apache.hadoop.ozone.recon.scm.ReconScmTask` | abstract | mixed | 50~ | 30 | Any background task that keeps SCM's metadata up to date. |
| 2381 | `org.apache.hadoop.ozone.recon.scm.ReconStorageContainerManagerFacade` | service | logic-heavy | 825~ | 60 | Recon's 'lite' version of SCM. |
| 2382 | `org.apache.hadoop.ozone.recon.scm.ReconStorageContainerSyncHelper` | service | logic-heavy | 375~ | 45 | Helper class that performs targeted incremental sync between SCM and Recon container metadata. |
| 2383 | `org.apache.hadoop.ozone.recon.scm.ReconContainerManager` | service | logic-heavy | 325~ | 45 | Recon's overriding implementation of SCM's Container Manager. |
| 2384 | `org.apache.hadoop.ozone.recon.scm.ReconNodeManager` | service | logic-heavy | 200~ | 45 | Recon SCM's Node manager that includes persistence. |
| 2385 | `org.apache.hadoop.ozone.recon.scm.ReconPipelineManager` | service | mixed | 100~ | 30 | Recon's overriding implementation of SCM's Pipeline Manager. |
| 2386 | `org.apache.hadoop.ozone.recon.scm.PipelineSyncTask` | service | mixed | 75~ | 30 | Background pipeline sync task that queries pipelines in SCM, and removes any obsolete pipeline. |
| 2387 | `org.apache.hadoop.ozone.recon.scm.ContainerReplicaHistory` | service | mixed | 75~ | 30 | A ContainerReplica timestamp class that tracks first and last seen time. |
| 2388 | `org.apache.hadoop.ozone.recon.scm.ReconPipelineReportHandler` | service | mixed | 50~ | 30 | Recon's implementation of Pipeline Report handler. |
| 2389 | `org.apache.hadoop.ozone.recon.scm.ReconDeadNodeHandler` | service | mixed | 50~ | 30 | Recon's handling of Dead node. |
| 2390 | `org.apache.hadoop.ozone.recon.scm.ReconPolicyProvider` | service | mixed | 25~ | 30 | PolicyProvider for Recon protocols. |
| 2391 | `org.apache.hadoop.ozone.recon.scm.ContainerReplicaHistoryList` | service | mixed | 25~ | 30 | A list of ContainerReplicaHistory. |
| 2392 | `org.apache.hadoop.ozone.recon.scm.ReconContainerReportQueue` | service | mixed | 25~ | 30 | Customized queue to handle multiple ICR report together. |
| 2393 | `org.apache.hadoop.ozone.recon.scm.ReconSCMDBDefinition` | service | mixed | 25~ | 30 | Recon SCM db file for ozone. |
| 2394 | `org.apache.hadoop.ozone.recon.scm.ReconContainerReportHandler` | service | mixed | 25~ | 30 | Recon's container report handler. |
| 2395 | `org.apache.hadoop.ozone.recon.scm.ReconDatanodeProtocolServer` | service | mixed | 25~ | 30 | Recon's Datanode protocol server extended from SCM. |
| 2396 | `org.apache.hadoop.ozone.recon.scm.ReconSafeModeManager` | service | mixed | 25~ | 30 | Recon's stub implementation of SCM's SafeMode manager. |
| 2397 | `org.apache.hadoop.ozone.recon.scm.ReconNewNodeHandler` | service | mixed | 25~ | 30 | Adds the new node to Recon Node DB. |
| 2398 | `org.apache.hadoop.ozone.recon.scm.ReconStaleNodeHandler` | service | mixed | 25~ | 30 | Recon's handling of Stale node. |
| 2399 | `org.apache.hadoop.ozone.recon.scm.ReconIncrementalContainerReportHandler` | service | mixed | 25~ | 30 | Recon ICR handler. |
| 2400 | `org.apache.hadoop.ozone.recon.scm.ReconStorageConfig` | config | data-only | 50~ | 20 | Recon's extension of SCMStorageConfig. |
| 2401 | `org.apache.hadoop.ozone.recon.scm.ReconPipelineFactory` | factory | mixed | 25~ | 20 | Class to stub out SCM's pipeline providers. |
...
- HDDS-14730. Update Recon container sync to use container IDs
- HDDS-14758. Mismatch in Recon container count between Cluster State and Container Summary
...
- HDDS-15413. Recon and SCM Container Sync Metrics addition

## Sharp edges

- `ReconStorageContainerSyncHelper` skips `CLOSING` and `DELETING` intermediate states by design. If a container transitions through these states and back to `OPEN` between sync cycles, Recon may miss the intermediate states and end up with an inconsistent view. (HDDS-14758 fixed a related count mismatch.)
- `ReconContainerManager` stores replica checksums in a `ConcurrentHashMap` that is not persisted; on Recon restart the checksum map is empty until ICR reports repopulate it. Any checksum-mismatch API call immediately after restart will return no mismatches. (HDDS-11887)
