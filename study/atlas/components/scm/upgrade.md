...
|--:|---|---|---|--:|--:|---|
| 1511 | `org.apache.hadoop.hdds.scm.server.upgrade.FinalizationManager` | interface | mixed | 25~ | 20 | Class to initiate SCM finalization and query its progress. |
| 1512 | `org.apache.hadoop.hdds.scm.server.upgrade.FinalizationStateManager` | interface | mixed | 25~ | 20 | Manages the state of finalization in SCM. |
| 1513 | `org.apache.hadoop.hdds.scm.server.upgrade.FinalizationStateManagerImpl` | service | logic-heavy | 200~ | 45 | Manages the state of finalization in SCM. |
| 1514 | `org.apache.hadoop.hdds.scm.server.upgrade.FinalizationManagerImpl` | service | mixed | 150~ | 45 | Class to initiate SCM finalization and query its progress. |
| 1515 | `org.apache.hadoop.hdds.scm.server.upgrade.SCMUpgradeFinalizationContext` | service | mixed | 100~ | 30 | Provided to methods in the SCMUpgradeFinalizer to supply objects needed to operate. |
| 1516 | `org.apache.hadoop.hdds.scm.server.upgrade.SCMUpgradeFinalizer` | service | mixed | 100~ | 30 | UpgradeFinalizer for the Storage Container Manager service. |
| 1517 | `org.apache.hadoop.hdds.scm.server.upgrade.ScmOnFinalizeActionForDatanodeSchemaV2` | service | mixed | 25~ | 30 | SCM Upgrade Action for the very first Upgrade Version. |
| 1518 | `org.apache.hadoop.hdds.scm.server.upgrade.FinalizationCheckpoint` | data | data-only | 25~ | 10 | A finalization checkpoint is an abstraction over SCM's disk state, indicating where finalization left off so it can b... |
...
- HDDS-12033. ScmHAUnfinalizedStateValidationAction can be removed as it's not used.
- HDDS-14569. Remove support for upgrade actions that run outside of finalization.
- HDDS-15191. Add ScmInvoker subclasses for the remaining SCMHandler(s) — includes `FinalizationStateManagerInvoker`.
