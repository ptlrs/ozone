...
|--:|---|---|---|--:|--:|---|
| 1 | `org.apache.hadoop.ozone.client.protocol.ClientProtocol` | interface | logic-heavy | 325~ | 20 | An implementer of this interface is capable of connecting to Ozone Cluster and perform client operations. |
| 2 | `org.apache.hadoop.ozone.client.rpc.RpcClient` | service | logic-heavy | 2325~ | 60 | Ozone RPC Client Implementation, it connects to OM, SCM and DataNode to execute client calls. |
| 3 | `org.apache.hadoop.ozone.client.OzoneClientUtils` | service | mixed | 150~ | 45 | Shared Utilities for Ozone FS and related classes. |
| 4 | `org.apache.hadoop.ozone.client.OzoneClient` | service | mixed | 25~ | 30 | OzoneClient connects to Ozone Cluster and perform basic operations. |
| 5 | `org.apache.hadoop.ozone.client.OzoneClientFactory` | factory | mixed | 125~ | 20 | Factory class to create OzoneClients. |
| 6 | `org.apache.hadoop.ozone.client.OzoneClientException` | exception | data-only | 25~ | 10 | This exception is thrown by the Ozone Clients. |

### Sub-feature: `user-api-types`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 7 | `org.apache.hadoop.ozone.client.OzoneVolume` | service | logic-heavy | 250~ | 20 | A class that encapsulates OzoneVolume. |
| 8 | `org.apache.hadoop.ozone.client.OzoneBucket` | service | logic-heavy | 950~ | 60 | A class that encapsulates OzoneBucket. |
| 9 | `org.apache.hadoop.ozone.client.ObjectStore` | service | logic-heavy | 375~ | 45 | ObjectStore class is responsible for the client operations that can be performed on Ozone Object Store. |
| 10 | `org.apache.hadoop.ozone.client.OzoneLifecycleConfiguration` | service | mixed | 150~ | 45 | A class that encapsulates OzoneLifecycleConfiguration. |
| 11 | `org.apache.hadoop.ozone.client.BucketArgs` | service | mixed | 150~ | 45 | This class encapsulates the arguments that are required for creating a bucket. |
| 12 | `org.apache.hadoop.ozone.client.OzoneSnapshot` | service | mixed | 125~ | 30 | A class that encapsulates OzoneSnapshot. |
| 13 | `org.apache.hadoop.ozone.client.OzoneKey` | service | mixed | 100~ | 30 | A class that encapsulates OzoneKey. |
| 14 | `org.apache.hadoop.ozone.client.VolumeArgs` | service | mixed | 75~ | 30 | This class encapsulates the arguments that are required for creating a volume. |
| 15 | `org.apache.hadoop.ozone.client.OzoneMultipartUploadPartListParts` | service | mixed | 75~ | 30 | Class that represents Multipart upload List parts response. |
| 16 | `org.apache.hadoop.ozone.client.OzoneMultipartUpload` | service | mixed | 75~ | 30 | Information about one initialized upload. |
| 17 | `org.apache.hadoop.ozone.client.OzoneSnapshotDiff` | service | mixed | 50~ | 30 | A class that encapsulates SnapshotDiffJob. |
| 18 | `org.apache.hadoop.ozone.client.OzoneKeyDetails` | service | mixed | 50~ | 30 | A class that encapsulates OzoneKeyLocation. |
| 19 | `org.apache.hadoop.ozone.client.TenantArgs` | service | mixed | 25~ | 30 | This class encapsulates the arguments for creating a tenant. |
| 20 | `org.apache.hadoop.ozone.client.OzoneMultipartUploadList` | service | mixed | 25~ | 30 | List of in-flight MPU upoads. |
| 21 | `org.apache.hadoop.ozone.client.OzoneKeyLocation` | service | mixed | 25~ | 30 | One key can be stored in one or more containers as one or more blocks. |

### Sub-feature: `io-streams`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 22 | `org.apache.hadoop.ozone.client.io.KeyOutputStream` | service | logic-heavy | 575~ | 60 | Maintaining a list of BlockInputStream. |
| 23 | `org.apache.hadoop.ozone.client.io.KeyDataStreamOutput` | service | logic-heavy | 375~ | 45 | Maintaining a list of BlockInputStream. |
| 24 | `org.apache.hadoop.ozone.client.io.BlockOutputStreamEntry` | service | logic-heavy | 275~ | 45 | A BlockOutputStreamEntry manages the data writes into the DataNodes. |
| 25 | `org.apache.hadoop.ozone.client.io.BlockOutputStreamEntryPool` | service | logic-heavy | 250~ | 45 | A BlockOutputStreamEntryPool manages the communication with OM during writing a Key to Ozone with KeyOutputStream. |
| 26 | `org.apache.hadoop.ozone.client.io.KeyInputStream` | service | mixed | 125~ | 30 | Maintaining a list of BlockInputStream. |
| 27 | `org.apache.hadoop.ozone.client.io.OzoneDataStreamOutput` | service | mixed | 125~ | 30 | OzoneDataStreamOutput is used to write data into Ozone. |
| 28 | `org.apache.hadoop.ozone.client.io.OzoneOutputStream` | service | mixed | 100~ | 30 | OzoneOutputStream is used to write data into Ozone. |
| 29 | `org.apache.hadoop.ozone.client.io.OzoneCryptoInputStream` | service | mixed | 100~ | 30 | A CryptoInputStream for Ozone with length. |
| 30 | `org.apache.hadoop.ozone.client.io.OzoneInputStream` | service | mixed | 75~ | 30 | OzoneInputStream is used to read data from Ozone. |
| 31 | `org.apache.hadoop.ozone.client.io.KeyOutputStreamSemaphore` | service | mixed | 50~ | 30 | Helper class that streamlines request semaphore usage in KeyOutputStream. |
| 32 | `org.apache.hadoop.ozone.client.io.CipherOutputStreamOzone` | service | mixed | 25~ | 30 | Wrap javax.crypto.CipherOutputStream with the method to return wrapped output stream. |

### Sub-feature: `client.io`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 33 | `org.apache.hadoop.ozone.client.io.KeyMetadataAware` | interface | mixed | 25~ | 20 | Interface to provide methods related to key metadata. |
| 34 | `org.apache.hadoop.ozone.client.io.KeyCommitOutput` | interface | mixed | 25~ | 20 | Common commit-time behavior for key output implementations. |
| 35 | `org.apache.hadoop.ozone.client.io.BlockDataStreamOutputEntryPool` | service | logic-heavy | 225~ | 45 | This class manages the stream entries list and handles block allocation from OzoneManager. |
| 36 | `org.apache.hadoop.ozone.client.io.BlockDataStreamOutputEntry` | service | logic-heavy | 200~ | 45 | Helper class used inside BlockDataStreamOutput. |

### Sub-feature: `client.rpc`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 37 | `org.apache.hadoop.ozone.client.rpc.OzoneKMSUtil` | service | mixed | 125~ | 30 | KMS utility class for Ozone Data Encryption At-Rest. |

### Sub-feature: `ec-client`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 38 | `org.apache.hadoop.ozone.client.io.ECKeyOutputStream` | service | logic-heavy | 525~ | 60 | ECKeyOutputStream handles the EC writes by writing the data into underlying block output streams chunk by chunk. |
| 39 | `org.apache.hadoop.ozone.client.io.ECBlockOutputStreamEntry` | service | logic-heavy | 275~ | 45 | ECBlockOutputStreamEntry manages write into EC keys' data block groups. |
| 40 | `org.apache.hadoop.ozone.client.checksum.ECBlockChecksumComputer` | service | mixed | 100~ | 30 | The implementation of AbstractBlockChecksumComputer for EC blocks. |
| 41 | `org.apache.hadoop.ozone.client.checksum.ECFileChecksumHelper` | service | mixed | 75~ | 30 | The helper class to compute file checksum for EC files. |
| 42 | `org.apache.hadoop.ozone.client.io.ECBlockOutputStreamEntryPool` | service | mixed | 25~ | 30 | BlockOutputStreamEntryPool is responsible to manage OM communication regarding writing a block to Ozone in a non-EC w... |

### Sub-feature: `checksum`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 43 | `org.apache.hadoop.ozone.client.checksum.BaseFileChecksumHelper` | abstract | logic-heavy | 250~ | 45 | The base class to support file checksum. |
| 44 | `org.apache.hadoop.ozone.client.checksum.AbstractBlockChecksumComputer` | abstract | mixed | 25~ | 30 | Base class for computing block checksum which is a function of chunk checksums. |
| 45 | `org.apache.hadoop.ozone.client.checksum.ReplicatedBlockChecksumComputer` | service | mixed | 100~ | 30 | The implementation of AbstractBlockChecksumComputer for replicated blocks. |
| 46 | `org.apache.hadoop.ozone.client.checksum.ReplicatedFileChecksumHelper` | service | mixed | 50~ | 30 | The helper class to compute file checksum for replicated files. |
| 47 | `org.apache.hadoop.ozone.client.checksum.ChecksumHelperFactory` | factory | mixed | 25~ | 20 | Class to create BaseFileChecksumHelper based on the Replication Type. |
...
- `hadoop-hdds/docs/content/design/ec.md` — Erasure Coding design covering `ECKeyOutputStream`, `ECBlockOutputStreamEntry`, and the overall EC write/read client architecture (HDDS-3816).
- `hadoop-hdds/docs/content/design/s3-performance.md` — S3 Gateway persistent OM connection design relevant to `RpcClient` OM stub lifecycle and connection reuse (HDDS-4440).
...
- HDDS-15193. Move the "atomic key creation" logic from output stream to S3 endpoints (refactored `KeyOutputStream` commit path).
- HDDS-15213. Use common commit output for stream outputs (`KeyCommitOutput` interface introduced).
- HDDS-13919. S3 Conditional Writes / PutObject (`RpcClient` conditional-create support).
- HDDS-13400. S3g accumulated memory pressure due to unlimited `ElasticByteBufferPool` in `RpcClient` (memory cap added).
- HDDS-15522. Make `RpcClient.close` idempotent.
- HDDS-14467. Expose pending delete bytes/namespace in `OzoneBucket`.
- HDDS-12782. Implement create/delete/get for bucket lifecycle configurations in OM (`OzoneLifecycleConfiguration` introduced).
...
Answer 2: A `SynchronousQueue` has zero capacity, so a `submit` blocks the caller until a worker thread picks up the task. With an unbounded `LinkedBlockingQueue`, submissions never block but memory grows without limit — the scenario fixed by HDDS-13400.
