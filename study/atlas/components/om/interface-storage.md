...
|--:|---|---|---|--:|--:|---|
| 470 | `org.apache.hadoop.ozone.om.helpers.OzoneAclStorageUtil` | service | mixed | 25~ | 30 | Helper class for ozone acls operations. |
| 471 | `org.apache.hadoop.ozone.om.helpers.OzoneAclStorage` | service | mixed | 25~ | 30 | OzoneAclStorage classes define bucket ACLs used in OZONE. |
| 472 | `org.apache.hadoop.ozone.om.helpers.OmPrefixInfo` | dto | data-only | 150~ | 10 | Wrapper class for Ozone prefix path info, currently mainly target for ACL but can be extended for other OzFS optimiza... |

### Sub-feature: `om.lock`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 473 | `org.apache.hadoop.ozone.om.lock.IOzoneManagerLock` | interface | mixed | 100~ | 20 | Interface for OM Metadata locks. |
| 474 | `org.apache.hadoop.ozone.om.lock.HierarchicalResourceLockManager` | interface | mixed | 25~ | 20 | Interface for Hierachical Resource Lock where the lock order acquired on resource is going to be deterministic and th... |
| 475 | `org.apache.hadoop.ozone.om.lock.OMLockDetails` | service | mixed | 75~ | 30 | This class is for recording detailed consumed time on Locks. |
| 476 | `org.apache.hadoop.ozone.om.lock.DAGLeveledResource` | data | data-only | 25~ | 10 | The DAGLeveledResource enum represents a set of resources that are associated with specific locks in the Ozone Manage... |
| 477 | `org.apache.hadoop.ozone.om.lock.LockUsageInfo` | dto | data-only | 25~ | 10 | Maintains lock related information useful in updating OMLockMetrics. |
| 478 | `org.apache.hadoop.ozone.om.lock.OMLockMetrics` | metrics | mixed | 75~ | 20 | This class is for maintaining the various Ozone Manager Lock Metrics. |

### Sub-feature: `ozone.om`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 479 | `org.apache.hadoop.ozone.om.OMMetadataManager` | interface | mixed | 150~ | 20 | OM metadata manager interface. |
| 480 | `org.apache.hadoop.ozone.om.ExpiredOpenKeys` | service | mixed | 25~ | 30 | The expired open keys. |
...
- HDDS-15907. Do not use Collection in IOzoneManagerLock (interface change)
...
- `IOzoneManagerLock` was changed to not use `Collection` in HDDS-15907. Any code that previously accepted `Collection<OMLockDetails>` for batch release must be updated to use the new typed signature.
...
4. `IOzoneManagerLock` was changed in HDDS-15907. What was the previous signature and why was it changed?
...
Answer 4: The previous signature used `Collection<OMLockDetails>` as a return type, which did not enforce ordering. HDDS-15907 changed it to avoid `Collection` since the lock release order matters and must match the acquisition order.
