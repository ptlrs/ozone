...
|--:|---|---|---|--:|--:|---|
| 390 | `org.apache.hadoop.ozone.container.common.transport.server.ratis.LocalStream` | state-machine | mixed | 25~ | 90 | inferred: LocalStream — role not documented. |
| 391 | `org.apache.hadoop.ozone.container.common.transport.server.ratis.ContainerStateMachine` | service | logic-heavy | 1000~ | 60 | A StateMachine for containers, which is responsible for handling different types of container requests. |
| 392 | `org.apache.hadoop.ozone.container.common.transport.server.ratis.XceiverServerRatis` | service | logic-heavy | 675~ | 60 | Creates a ratis server endpoint that acts as the communication layer for Ozone containers. |
| 393 | `org.apache.hadoop.ozone.container.common.transport.server.ratis.RatisServerConfiguration` | service | mixed | 25~ | 30 | Holds configuration items for Ratis/Raft server. |
| 394 | `org.apache.hadoop.ozone.container.common.transport.server.ratis.DispatcherContext` | service | mixed | 150~ | 10 | DispatcherContext class holds transport protocol specific context info required for execution of container commands o... |
| 395 | `org.apache.hadoop.ozone.container.common.transport.server.ratis.CSMMetrics` | metrics | mixed | 175~ | 20 | This class is for maintaining Container State Machine statistics. |

### Sub-feature: `statemachine.background`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 396 | `org.apache.hadoop.ozone.container.keyvalue.statemachine.background.BlockDeletingTask` | service | logic-heavy | 400~ | 60 | BlockDeletingTask for KeyValueContainer. |
| 397 | `org.apache.hadoop.ozone.container.keyvalue.statemachine.background.StaleRecoveringContainerScrubbingService` | service | mixed | 50~ | 30 | A per-datanode container stale recovering container scrubbing service takes in charge of deleting stale recovering co... |
...
- HDDS-15443. Close state machine on write failure.
...
- `BlockDeletingTask` deletes chunk files before removing the RocksDB entry; if the datanode crashes between the file delete and the RocksDB batch commit, the block metadata entry remains but the data is gone — the container scanner marks it UNHEALTHY on the next scan (HDDS-15291).
...
Answer 1: Ratis treats the exception as a write failure and closes the pipeline; the leader's `notifyLogFailed()` callback is invoked, which eventually causes SCM to re-create the pipeline (HDDS-15443 adds explicit state machine close on this path).
