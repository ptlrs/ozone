...
|--:|---|---|---|--:|--:|---|
| 1544 | `org.apache.hadoop.ozone.container.common.impl.ContainerData` | abstract | logic-heavy | 400~ | 60 | ContainerData is the in-memory representation of container metadata and is represented on disk by the .container file. |
| 1545 | `org.apache.hadoop.ozone.container.common.impl.HddsDispatcher` | service | logic-heavy | 800~ | 60 | Ozone Container dispatcher takes a call from the netty server and routes it to the right handler function. |
| 1546 | `org.apache.hadoop.ozone.container.common.impl.ContainerSet` | service | logic-heavy | 375~ | 45 | Class that manages Containers created on the datanode. |
| 1547 | `org.apache.hadoop.ozone.container.common.impl.StorageLocationReport` | service | logic-heavy | 300~ | 45 | Storage location stats of datanodes that provide back store for containers. |
| 1548 | `org.apache.hadoop.ozone.container.common.impl.BlockDeletingService` | service | logic-heavy | 250~ | 45 | A per-datanode container block deleting service takes in charge of deleting staled ozone blocks. |
| 1549 | `org.apache.hadoop.ozone.container.common.impl.ContainerDataYaml` | service | logic-heavy | 200~ | 45 | Class for creating and reading .container files. |
| 1550 | `org.apache.hadoop.ozone.container.common.impl.ContainerDataScanOrder` | service | mixed | 25~ | 30 | Orders containers: 1. |
| 1551 | `org.apache.hadoop.ozone.container.common.impl.RandomContainerDeletionChoosingPolicy` | service | mixed | 25~ | 30 | Randomly choosing containers for block deletion. |
| 1552 | `org.apache.hadoop.ozone.container.common.impl.TopNOrderedContainerDeletionChoosingPolicy` | service | mixed | 25~ | 30 | TopN Ordered choosing policy that choosing containers based on pending deletion blocks' number. |
| 1553 | `org.apache.hadoop.ozone.container.common.impl.ContainerLayoutVersion` | data | data-only | 75~ | 10 | Defines layout versions for the Chunks. |

### Sub-feature: `common.states`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1554 | `org.apache.hadoop.ozone.container.common.states.DatanodeState` | interface | mixed | 25~ | 20 | State Interface that allows tasks to maintain states. |

### Sub-feature: `container.common`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1555 | `org.apache.hadoop.ozone.container.common.DatanodeLayoutStorage` | service | mixed | 50~ | 30 | DataNodeStorageConfig is responsible for management of the StorageDirectories used by the DataNode. |
| 1556 | `org.apache.hadoop.ozone.container.common.HDDSVolumeLayoutVersion` | service | mixed | 25~ | 30 | Layout version which describes information about the layout version on the datanode volume. |

### Sub-feature: `container.ozoneimpl`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1557 | `org.apache.hadoop.ozone.container.ozoneimpl.AbstractBackgroundContainerScanner` | abstract | mixed | 125~ | 30 | Base class for scheduled scanners on a Datanode. |
| 1558 | `org.apache.hadoop.ozone.container.ozoneimpl.OzoneContainer` | service | logic-heavy | 525~ | 60 | Ozone main class sets up the network servers and initializes the container layer. |
| 1559 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerReader` | service | logic-heavy | 250~ | 45 | Class used to read .container files from Volume and build container map. |
| 1560 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerScanHelper` | service | mixed | 175~ | 45 | Mixin to handle common data and metadata scan operations among background and on-demand scanners. |
| 1561 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerScannerConfiguration` | service | mixed | 150~ | 45 | This class defines configuration parameters for the container scanners. |
| 1562 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerController` | service | mixed | 150~ | 45 | Control plane for container management in datanode. |
| 1563 | `org.apache.hadoop.ozone.container.ozoneimpl.OnDemandContainerScanner` | service | mixed | 100~ | 30 | Class for performing on demand scans of containers. |
| 1564 | `org.apache.hadoop.ozone.container.ozoneimpl.BackgroundContainerDataScanner` | service | mixed | 75~ | 30 | Data scanner that full checks a volume. |
| 1565 | `org.apache.hadoop.ozone.container.ozoneimpl.MetadataScanResult` | service | mixed | 50~ | 30 | Represents the result of a container metadata scan. |
| 1566 | `org.apache.hadoop.ozone.container.ozoneimpl.ScanTransientIOUtil` | service | mixed | 50~ | 30 | Utility to catch transient scan failures (typically related to file-descriptor exhaustion) that should not be treated... |
| 1567 | `org.apache.hadoop.ozone.container.ozoneimpl.DataScanResult` | service | mixed | 25~ | 30 | Represents the result of a container data scan. |
| 1568 | `org.apache.hadoop.ozone.container.ozoneimpl.BackgroundContainerMetadataScanner` | service | mixed | 25~ | 30 | This class is responsible to perform metadata verification of the containers. |
| 1569 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerScanError` | service | mixed | 25~ | 10 | This class is used to identify any error that may be seen while scanning a container. |
| 1570 | `org.apache.hadoop.ozone.container.ozoneimpl.AbstractContainerScannerMetrics` | metrics | mixed | 50~ | 20 | Base class for container scanner metrics. |
| 1571 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerDataScannerMetrics` | metrics | mixed | 50~ | 20 | This class captures the container data scanner metrics on the data-node. |
| 1572 | `org.apache.hadoop.ozone.container.ozoneimpl.ContainerMetadataScannerMetrics` | metrics | mixed | 25~ | 20 | This class captures the container meta-data scanner metrics on the data-node. |
| 1573 | `org.apache.hadoop.ozone.container.ozoneimpl.OnDemandScannerMetrics` | metrics | mixed | 25~ | 20 | This class captures the on-demand container data scanner metrics. |

### Sub-feature: `ozone`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1574 | `org.apache.hadoop.ozone.HddsDatanodeStopService` | interface | mixed | 25~ | 20 | Interface which declares a method to stop HddsDatanodeService. |
| 1575 | `org.apache.hadoop.ozone.DNMXBean` | interface | mixed | 25~ | 20 | This is the JMX management interface for DN information. |
| 1576 | `org.apache.hadoop.ozone.HddsDatanodeService` | cli | logic-heavy | 575~ | 60 | Datanode service plugin to start the HDDS container services. |
| 1577 | `org.apache.hadoop.ozone.HddsDatanodeClientProtocolServer` | service | mixed | 100~ | 30 | The RPC server that listens to requests from clients. |
| 1578 | `org.apache.hadoop.ozone.DNMXBeanImpl` | service | mixed | 50~ | 30 | This is the JMX management class for DN information. |
| 1579 | `org.apache.hadoop.ozone.HddsDatanodeHttpServer` | service | mixed | 50~ | 30 | Simple http server to provide basic monitoring for hdds datanode. |
| 1580 | `org.apache.hadoop.ozone.HddsPolicyProvider` | service | mixed | 25~ | 30 | PolicyProvider for Datanode protocols. |

### Sub-feature: `states.datanode`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1581 | `org.apache.hadoop.ozone.container.common.states.datanode.RunningDatanodeState` | service | mixed | 125~ | 30 | Class that implements handshake with SCM. |
| 1582 | `org.apache.hadoop.ozone.container.common.states.datanode.InitDatanodeState` | service | mixed | 75~ | 30 | Init Datanode State is the task that gets run when we are in Init State. |

### Sub-feature: `states.endpoint`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1583 | `org.apache.hadoop.ozone.container.common.states.endpoint.HeartbeatEndpointTask` | service | logic-heavy | 350~ | 45 | Heartbeat class for SCMs. |
| 1584 | `org.apache.hadoop.ozone.container.common.states.endpoint.RegisterEndpointTask` | service | mixed | 150~ | 45 | Register a datanode with SCM. |
| 1585 | `org.apache.hadoop.ozone.container.common.states.endpoint.VersionEndpointTask` | service | mixed | 75~ | 30 | Task that returns version. |
...
`ContainerReader.run()` iterates over the container directories on a single `HddsVolume`, reads each `.container` YAML file via `ContainerDataYaml.readContainerFile()`, and calls `ContainerSet.addContainer()`. HDDS-15455 added duplicate-detection logic so that a container found in multiple directories is not double-counted. A `ContainerReader` is launched per volume in parallel during `OzoneContainer.start()`.
...
- HDDS-12669. Race condition between entries of `ContainerSet#recoveringContainerMap`.
...
- `ContainerSet.recoveringContainerMap` is a secondary index used during EC recovery; HDDS-12669 fixed a race where concurrent inserts/removes left a stale reference to a deleted container, causing NPE in scanner code. Access requires the `ContainerSet` write lock.
- `HddsDispatcher.dispatch()` marks a container UNHEALTHY on scan errors; HDDS-15150 added a `ScanTransientIOUtil` check so that transient FD-exhaustion errors do not permanently blacklist a healthy container.
...
Answer 1: Prior to HDDS-15455 a duplicate container ID on two volumes caused the second to silently overwrite the first in `ContainerSet`; HDDS-15455 added detection logic that logs an error and excludes the duplicate.
