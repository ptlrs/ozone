...
|--:|---|---|---|--:|--:|---|
| 925 | `org.apache.hadoop.ozone.container.replication.ContainerReplicator` | interface | mixed | 25~ | 20 | Service to do the real replication task. |
| 926 | `org.apache.hadoop.ozone.container.replication.ContainerReplicationSource` | interface | mixed | 25~ | 20 | Contract to prepare provide the container in binary form.. |
| 927 | `org.apache.hadoop.ozone.container.replication.ContainerUploader` | interface | mixed | 25~ | 20 | Client-side interface for sending a container to a target datanode. |
| 928 | `org.apache.hadoop.ozone.container.replication.GrpcOutputStream` | abstract | mixed | 125~ | 30 | Adapter from OutputStream to gRPC StreamObserver. |
| 929 | `org.apache.hadoop.ozone.container.replication.ReplicationSupervisor` | service | logic-heavy | 525~ | 60 | Single point to schedule container replication tasks based on priorities. |
| 930 | `org.apache.hadoop.ozone.container.replication.ReplicationServer` | service | logic-heavy | 250~ | 45 | Separated network server for server2server container replication. |
| 931 | `org.apache.hadoop.ozone.container.replication.GrpcContainerUploader` | service | mixed | 150~ | 45 | Uploads container to target datanode via gRPC. |
| 932 | `org.apache.hadoop.ozone.container.replication.VolumeReplicationThreadPools` | service | mixed | 125~ | 30 | Per-volume replication handler thread pools for push-based replication. |
| 933 | `org.apache.hadoop.ozone.container.replication.ContainerImporter` | service | mixed | 125~ | 30 | Imports container from tarball. |
| 934 | `org.apache.hadoop.ozone.container.replication.SendContainerRequestHandler` | service | mixed | 100~ | 30 | Handles incoming container pushed by other datanode. |
| 935 | `org.apache.hadoop.ozone.container.replication.ReplicationTask` | service | mixed | 75~ | 30 | Task to push a container to a target datanode. |
| 936 | `org.apache.hadoop.ozone.container.replication.MeasuredReplicator` | service | mixed | 75~ | 30 | ContainerReplicator wrapper with additional metrics. |
| 937 | `org.apache.hadoop.ozone.container.replication.GrpcReplicationClient` | service | mixed | 50~ | 30 | Client to push container data to another datanode via gRPC. |
| 938 | `org.apache.hadoop.ozone.container.replication.PushReplicator` | service | mixed | 50~ | 30 | Pushes the container to the target datanode. |
| 939 | `org.apache.hadoop.ozone.container.replication.GrpcReplicationService` | service | mixed | 50~ | 30 | Service to make containers available for replication. |
| 940 | `org.apache.hadoop.ozone.container.replication.OnDemandContainerReplicationSource` | service | mixed | 25~ | 30 | A naive implementation of the replication source which creates a tar file on-demand without pre-create the compressed... |
| 941 | `org.apache.hadoop.ozone.container.replication.SendContainerOutputStream` | service | mixed | 25~ | 30 | Output stream adapter for SendContainerResponse. |
| 942 | `org.apache.hadoop.ozone.container.replication.AbstractReplicationTask` | abstract | mixed | 75~ | 10 | Abstract class to capture common variables and methods for different types of replication tasks. |
| 943 | `org.apache.hadoop.ozone.container.replication.CopyContainerCompression` | data | data-only | 75~ | 10 | Defines compression algorithm for container replication. |
| 944 | `org.apache.hadoop.ozone.container.replication.ReplicationSupervisorMetrics` | metrics | mixed | 75~ | 20 | Metrics source to report number of replication tasks. |
...
- HDDS-15614. Remove Datanode download/pull container replication (push-only now).
- HDDS-10237. Dynamic reconfiguration of replication supervisor thread pool.
- HDDS-12235. Reserve space on DN during container import operation.
- HDDS-12547. Container creation and import use the same `VolumeChoosingPolicy`.
- HDDS-13238. Scan container and volume during import/export and EC reconstruction.
- HDDS-15326. Clamp out-of-service limit factor to bounds instead of resetting to default.

## Sharp edges

- `ReplicationSupervisor` deduplicates tasks by (containerId, targetDN) key; if the same container needs replication to two different targets concurrently both tasks are submitted, but a second task to the same target is silently dropped while the first is in flight (HDDS-10237 covers pool resize; no explicit fix for the drop window).
- `ContainerImporter` calls `VolumeChoosingPolicy` to select the destination volume; if the target volume has insufficient space the import fails and no fallback to another volume is attempted — callers must pre-check via `AvailableSpaceFilter` (HDDS-12235).
...
Answer 5: `ContainerImporter` must verify that the destination volume has enough free space (checked via `AvailableSpaceFilter` / `HddsVolume.reserveSpace`); without the reservation, concurrent imports can overcommit a volume (HDDS-12235).
