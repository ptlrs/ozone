...
|--:|---|---|---|--:|--:|---|
| 777 | `org.apache.hadoop.hdds.utils.db.managed.ManagedRocksDB` | service | mixed | 100~ | 30 | Managed RocksDB. |
| 778 | `org.apache.hadoop.hdds.utils.db.managed.ManagedRocksObjectUtils` | service | mixed | 50~ | 30 | Utilities to help assert RocksObject closures. |
| 779 | `org.apache.hadoop.hdds.utils.db.managed.ManagedColumnFamilyOptions` | service | mixed | 50~ | 30 | Managed ColumnFamilyOptions. |
| 780 | `org.apache.hadoop.hdds.utils.db.managed.ManagedSstFileReaderIterator` | service | mixed | 25~ | 30 | Managed SstFileReaderIterator. |
| 781 | `org.apache.hadoop.hdds.utils.db.managed.ManagedCompactRangeOptions` | service | mixed | 25~ | 30 | Managed CompactRangeOptions. |
| 782 | `org.apache.hadoop.hdds.utils.db.managed.ManagedReadOptions` | service | mixed | 25~ | 30 | Managed ReadOptions. |
| 783 | `org.apache.hadoop.hdds.utils.db.managed.ManagedDBOptions` | service | mixed | 25~ | 30 | Managed DBOptions. |
| 784 | `org.apache.hadoop.hdds.utils.db.managed.ManagedFlushOptions` | service | mixed | 25~ | 30 | Managed FlushOptions. |
| 785 | `org.apache.hadoop.hdds.utils.db.managed.ManagedTransactionLogIterator` | service | mixed | 25~ | 30 | Managed TransactionLogIterator. |
| 786 | `org.apache.hadoop.hdds.utils.db.managed.ManagedRocksIterator` | service | mixed | 25~ | 30 | Managed RocksIterator. |
| 787 | `org.apache.hadoop.hdds.utils.db.managed.ManagedCheckpoint` | service | mixed | 25~ | 30 | Managed Checkpoint. |
| 788 | `org.apache.hadoop.hdds.utils.db.managed.ManagedDirectSlice` | service | mixed | 25~ | 30 | Managed DirectSlice for leak detection. |
| 789 | `org.apache.hadoop.hdds.utils.db.managed.ManagedLogger` | service | mixed | 25~ | 30 | Managed Logger. |
| 790 | `org.apache.hadoop.hdds.utils.db.managed.JniLibNamePropertyWriter` | service | mixed | 25~ | 30 | Class to write the rocksdb lib name to a file. |
| 791 | `org.apache.hadoop.hdds.utils.db.managed.ManagedStatistics` | service | mixed | 25~ | 30 | Managed Statistics. |
| 792 | `org.apache.hadoop.hdds.utils.db.managed.ManagedWriteBatch` | service | mixed | 25~ | 30 | Managed WriteBatch. |
| 793 | `org.apache.hadoop.hdds.utils.db.managed.ManagedIngestExternalFileOptions` | service | mixed | 25~ | 30 | Managed IngestExternalFileOptions. |
| 794 | `org.apache.hadoop.hdds.utils.db.managed.ManagedBloomFilter` | service | mixed | 25~ | 30 | Managed BloomFilter. |
| 795 | `org.apache.hadoop.hdds.utils.db.managed.ManagedObject` | service | mixed | 25~ | 30 | General template for a managed RocksObject. |
| 796 | `org.apache.hadoop.hdds.utils.db.managed.ManagedLRUCache` | service | mixed | 25~ | 30 | Managed LRUCache. |
| 797 | `org.apache.hadoop.hdds.utils.db.managed.ManagedSlice` | service | mixed | 25~ | 30 | Managed Slice. |
| 798 | `org.apache.hadoop.hdds.utils.db.managed.ManagedConfigOptions` | service | mixed | 25~ | 30 | Managed ConfigOptions. |
| 799 | `org.apache.hadoop.hdds.utils.db.managed.ManagedSstFileWriter` | service | mixed | 25~ | 30 | Managed SstFileWriter. |
| 800 | `org.apache.hadoop.hdds.utils.db.managed.ManagedEnvOptions` | service | mixed | 25~ | 30 | Managed EnvOptions. |
| 801 | `org.apache.hadoop.hdds.utils.db.managed.ManagedOptions` | service | mixed | 25~ | 30 | Managed Options. |
| 802 | `org.apache.hadoop.hdds.utils.db.managed.ManagedWriteOptions` | service | mixed | 25~ | 30 | Managed WriteOptions. |
| 803 | `org.apache.hadoop.hdds.utils.db.managed.ManagedSstFileReader` | service | mixed | 25~ | 30 | Managed SstFileReader. |
| 804 | `org.apache.hadoop.hdds.utils.db.managed.ManagedBlockBasedTableConfig` | config | data-only | 25~ | 20 | Managed BlockBasedTableConfig. |
| 805 | `org.apache.hadoop.hdds.utils.db.managed.ManagedRocksObjectMetrics` | metrics | mixed | 50~ | 20 | Metrics about managed RockObjects. |

### Sub-feature: `utils.db`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 806 | `org.apache.hadoop.hdds.utils.db.RocksDatabaseException` | exception | data-only | 25~ | 10 | Exceptions converted from RocksDBException. |
...
- `hadoop-hdds/docs/content/design/dn-merge-rocksdb.md` — design for merging per-container RocksDB instances into a single DN-level DB (HDDS-3630), which relies on the managed wrappers for safe multi-handle lifecycle management.
- No dedicated design doc for the managed-wrapper pattern itself under `hadoop-hdds/docs/content/` on this branch.

## Seminal JIRAs / PRs

- HDDS-11543. Track OzoneClient object leaks via LeakDetector framework (established the leak-tracking pattern used here).
- HDDS-12830. Add RocksDatabaseException.
- HDDS-12173. Follow RocksDB basic tuning guide (and its revert — shows fragility of bulk option changes).
- HDDS-13422. Ozone tool should preserve previous RocksDB options (motivated `openWithLatestOptions`).
- HDDS-14237. Simplify ManagedDirectSlice to use ByteBuffers.
