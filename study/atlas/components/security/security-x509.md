...
|--:|---|---|---|--:|--:|---|
| 999 | `org.apache.hadoop.hdds.security.x509.certificate.authority.profile.PKIProfile` | interface | mixed | 25~ | 20 | Base class for profile rules. |
| 1000 | `org.apache.hadoop.hdds.security.x509.certificate.authority.profile.DefaultProfile` | service | mixed | 175~ | 45 | Ozone PKI profile. |
| 1001 | `org.apache.hadoop.hdds.security.x509.certificate.authority.profile.DefaultCAProfile` | service | mixed | 25~ | 30 | CA Profile, this is needed when SCM does HA. |

### Sub-feature: `certificate.authority`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1002 | `org.apache.hadoop.hdds.security.x509.certificate.authority.CertificateStore` | interface | mixed | 25~ | 20 | This interface allows the DefaultCA to be portable and use different DB interfaces later. |
| 1003 | `org.apache.hadoop.hdds.security.x509.certificate.authority.CertificateApprover` | interface | mixed | 25~ | 20 | Certificate Approver interface is used to inspectCSR a certificate. |
| 1004 | `org.apache.hadoop.hdds.security.x509.certificate.authority.CertificateServer` | interface | mixed | 25~ | 20 | Interface for Certificate Authority. |
| 1005 | `org.apache.hadoop.hdds.security.x509.certificate.authority.DefaultCAServer` | service | logic-heavy | 300~ | 45 | The default CertificateServer used by SCM. |
| 1006 | `org.apache.hadoop.hdds.security.x509.certificate.authority.DefaultApprover` | service | mixed | 175~ | 45 | Default Approver used the by the DefaultCA. |

### Sub-feature: `certificate.client`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1007 | `org.apache.hadoop.hdds.security.x509.certificate.client.CertificateClient` | interface | mixed | 50~ | 20 | Certificate client provides and interface to certificate operations that needs to be performed by all clients in the... |
| 1008 | `org.apache.hadoop.hdds.security.x509.certificate.client.CertificateNotification` | interface | mixed | 25~ | 20 | Class should implement this interface if it wants to be notified when there is some changes in Certificate. |
| 1009 | `org.apache.hadoop.hdds.security.x509.certificate.client.DefaultCertificateClient` | abstract | logic-heavy | 1025~ | 60 | Default Certificate client implementation. |
| 1010 | `org.apache.hadoop.hdds.security.x509.certificate.client.SCMCertificateClient` | service | logic-heavy | 250~ | 45 | SCM Certificate Client which is used for generating public/private Key pair, generate CSR and finally obtain signed c... |
| 1011 | `org.apache.hadoop.hdds.security.x509.certificate.client.RootCaRotationPoller` | service | mixed | 100~ | 30 | Poller mechanism for Root Ca Rotation for clients. |
| 1012 | `org.apache.hadoop.hdds.security.x509.certificate.client.DNCertificateClient` | service | mixed | 50~ | 30 | Certificate client for DataNodes. |

### Sub-feature: `certificate.utils`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1013 | `org.apache.hadoop.hdds.security.x509.certificate.utils.SelfSignedCertificate` | service | logic-heavy | 200~ | 45 | A Self Signed Certificate with CertificateServer basic constraint can be used to bootstrap a certificate infrastructu... |
| 1014 | `org.apache.hadoop.hdds.security.x509.certificate.utils.CertificateSignRequest` | dto | logic-heavy | 275~ | 10 | A certificate sign request object that wraps operations to build a PKCS10CertificationRequest to CertificateServer. |

### Sub-feature: `x509.certificate`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 1015 | `org.apache.hadoop.hdds.security.x509.certificate.CertInfo` | dto | data-only | 100~ | 10 | Class that wraps Certificate Info. |
...
- HDDS-10249. Remove unused dead code in hdds-common module.
...
- HDDS-11031. Merge BaseApprover abstract class into DefaultApprover.
