---
slug: /component/ratis-integration/ratis-integration
...
_This feature page was generated from `study/atlas/components/ratis-integration/ratis-integration.md`. Author prose belongs above or below the BEGIN/END markers._
...
click SCMHandler href "/class/scmhandler" "Open SCMHandler"
click SCMHAUtils href "/class/scmhautils" "Open SCMHAUtils"
click SequenceIdType href "/class/sequenceidtype" "Open SequenceIdType"
click RatisMetricsUtils href "/class/ratismetricsutils" "Open RatisMetricsUtils"
```
...
|--:|---|---|---|--:|--:|---|
| 282 | <Fqcn>org.apache.ratis.metrics.dropwizard3.RatisMetricsUtils</Fqcn> | metrics | mixed | 25~ | 20 | Utilities for ratis metrics dropwizard3. |

### Sub-feature: `scm.ha`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 283 | <Fqcn>org.apache.hadoop.hdds.scm.ha.SCMHandler</Fqcn> | interface | mixed | 25~ | 20 | Base interface for SCM handlers participating in Ratis-based HA. |
| 284 | <Fqcn>org.apache.hadoop.hdds.scm.ha.SCMHAUtils</Fqcn> | service | mixed | 150~ | 45 | Utility class used by SCM HA. |
| 285 | <Fqcn>org.apache.hadoop.hdds.scm.ha.SequenceIdType</Fqcn> | data | data-only | 75~ | 10 | Represents the sequence ID types managed by <Fqcn>org.apache.hadoop.hdds.scm.ha.SequenceIdGenerator</Fqcn> The enum constant names... |
...
- [HDDS-15370](https://issues.apache.org/jira/browse/HDDS-15370). Change sequenceIdTable to use SequenceIdType
- [HDDS-14919](https://issues.apache.org/jira/browse/HDDS-14919). Add a base class for all the SCM handler interfaces
- [HDDS-13866](https://issues.apache.org/jira/browse/HDDS-13866). Use component-specific default directory for Ratis
- [HDDS-12351](https://issues.apache.org/jira/browse/HDDS-12351). Move SCMHAUtils and ServerUtils to hdds-server-framework
- [HDDS-13753](https://issues.apache.org/jira/browse/HDDS-13753). Use forked Hadoop RPC
- [HDDS-9192](https://issues.apache.org/jira/browse/HDDS-9192). Update Ratis to 3.0.0
...
- `rootCertificateId` in `SequenceIdType` is `@Deprecated` in favour of `CertificateId`, but both constants still exist and both have active RocksDB entries in older clusters. Any code that generates a new `rootCertificateId` sequence ID rather than `CertificateId` after the migration point will silently create a diverging counter. ([HDDS-15370](https://issues.apache.org/jira/browse/HDDS-15370) introduced the migration; verify upgrade finalization guards before touching this.)
