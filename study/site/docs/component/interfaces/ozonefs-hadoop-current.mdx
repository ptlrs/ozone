---
slug: /component/interfaces/ozonefs-hadoop-current
sidebar_label: "ozonefs-hadoop-current"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/interfaces/ozonefs-hadoop-current.md`. Author prose belongs above or below the BEGIN/END markers._
...
click PosixOzoneFileSystem href "/class/posixozonefilesystem" "Open PosixOzoneFileSystem"
click PosixRootedOzoneFileSystem href "/class/posixrootedozonefilesystem" "Open PosixRootedOzoneFileSystem"
```
...
|--:|---|---|---|--:|--:|---|
| 2525 | <Fqcn>org.apache.hadoop.fs.ozone.PosixRootedOzoneFileSystem</Fqcn> | service | mixed | 25~ | 30 | The PosixRootedOzoneFileSystem implementation. |
| 2526 | <Fqcn>org.apache.hadoop.fs.ozone.PosixOzoneFileSystem</Fqcn> | service | mixed | 25~ | 30 | The PosixOzoneFileSystem implementation. |
...
- [HDDS-14490](https://issues.apache.org/jira/browse/HDDS-14490). HadoopRpcOMFollowerProxyProvider should explicitly use OzoneManagerProtocolPB.
...
- [HDDS-13761](https://issues.apache.org/jira/browse/HDDS-13761). Remove hadoop-thirdparty protobuf compilation.
...
- `OzFs` and `RootedOzFs` implement `AbstractFileSystem` which is required for `FileContext` API usage. The `FileContext` lifecycle does not call `FileSystem.close()` on the underlying `FileSystem`; callers using `FileContext` must manage OzoneClient lifetime explicitly to avoid connection leaks (see [HDDS-11635](https://issues.apache.org/jira/browse/HDDS-11635)).
