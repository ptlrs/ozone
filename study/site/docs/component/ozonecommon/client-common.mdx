---
slug: /component/ozonecommon/client-common
sidebar_label: "client-common"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/ozonecommon/client-common.md`. Author prose belongs above or below the BEGIN/END markers._
...
click SelectorOutputStream href "/class/selectoroutputstream" "Open SelectorOutputStream"
click CrcUtil href "/class/crcutil" "Open CrcUtil"
click CrcComposer href "/class/crccomposer" "Open CrcComposer"
click CompositeCrcFileChecksum href "/class/compositecrcfilechecksum" "Open CompositeCrcFileChecksum"
click WrappedOutputStream href "/class/wrappedoutputstream" "Open WrappedOutputStream"
click LengthInputStream href "/class/lengthinputstream" "Open LengthInputStream"
```
...
|--:|---|---|---|--:|--:|---|
| 48 | <Fqcn>org.apache.hadoop.ozone.client.checksum.CrcUtil</Fqcn> | service | mixed | 125~ | 30 | This class provides utilities for working with CRCs. |
| 49 | <Fqcn>org.apache.hadoop.ozone.client.checksum.CrcComposer</Fqcn> | service | mixed | 100~ | 30 | Encapsulates logic for composing multiple CRCs into one or more combined CRCs corresponding to concatenated underlyin... |
| 50 | <Fqcn>org.apache.hadoop.ozone.client.checksum.CompositeCrcFileChecksum</Fqcn> | service | mixed | 50~ | 30 | Composite CRC. |

### Sub-feature: `client.io`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 51 | <Fqcn>org.apache.hadoop.ozone.client.io.SelectorOutputStream</Fqcn> | service | mixed | 125~ | 30 | An OutputStream first write data to a buffer up to the capacity. |
| 52 | <Fqcn>org.apache.hadoop.ozone.client.io.WrappedOutputStream</Fqcn> | service | mixed | 25~ | 30 | Pure wrapper of OutputStream. |
| 53 | <Fqcn>org.apache.hadoop.ozone.client.io.LengthInputStream</Fqcn> | service | mixed | 25~ | 30 | An input stream with length. |
...
- [HDDS-9228](https://issues.apache.org/jira/browse/HDDS-9228). Poor S3G read performance (introduced `SelectorOutputStream` to avoid early stream selection)
- [HDDS-10489](https://issues.apache.org/jira/browse/HDDS-10489). Use CRC tables to speed up galoisFieldMultiply in CrcUtil
- [HDDS-15115](https://issues.apache.org/jira/browse/HDDS-15115). CrcUtil/CrcComposer should not throw IOException for non-IO operations
- [HDDS-9886](https://issues.apache.org/jira/browse/HDDS-9886). `hadoop fs -checksum` fails with `NoClassDefFoundError` on Hadoop 2
- [HDDS-11816](https://issues.apache.org/jira/browse/HDDS-11816). Ozone stream to support Hsync/Hflush
...
4. What Galois Field operation does `CrcUtil.galoisFieldMultiply` perform and why was it accelerated in [HDDS-10489](https://issues.apache.org/jira/browse/HDDS-10489)?
...
Answer 4: `galoisFieldMultiply` computes polynomial multiplication over GF(2^32) used in CRC combination. [HDDS-10489](https://issues.apache.org/jira/browse/HDDS-10489) replaced the naive loop with a lookup table (stored as a static array in `CrcUtil`) to reduce CPU work per byte.
