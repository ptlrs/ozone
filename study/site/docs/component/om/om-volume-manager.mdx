---
slug: /component/om/om-volume-manager
sidebar_label: "om-volume-manager"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/om/om-volume-manager.md`. Author prose belongs above or below the BEGIN/END markers._
...
The `om-volume-manager` feature defines the volume-level read-path interface and its implementation. `VolumeManager` defines `getVolumeInfo`, `listVolumes`, `listAllVolumes`, and `checkAccess`. `VolumeManagerImpl` implements these against `OMMetadataManager`, using `OzoneManagerLock.VOLUME_LOCK` for read-path operations. Volume listing by admin uses the `userTable` index to find all volumes owned by a user. The `ozone.om.volume.listall.allowed` config controls whether non-admin users can list all volumes ([HDDS-12301](https://issues.apache.org/jira/browse/HDDS-12301), made reconfigurable in [HDDS-12248](https://issues.apache.org/jira/browse/HDDS-12248)). All write-path volume operations (create, delete, set-quota) are in `om-request-volume`.
...
click VolumeManager href "/class/volumemanager" "Open VolumeManager"
click VolumeManagerImpl href "/class/volumemanagerimpl" "Open VolumeManagerImpl"
```
...
|--:|---|---|---|--:|--:|---|
| 483 | <Fqcn>org.apache.hadoop.ozone.om.VolumeManager</Fqcn> | interface | mixed | 25~ | 20 | VolumeManager is responsible for read operations on a volume. |
| 484 | <Fqcn>org.apache.hadoop.ozone.om.VolumeManagerImpl</Fqcn> | service | mixed | 100~ | 30 | Volume Manager implementation. |
...
- [HDDS-12301](https://issues.apache.org/jira/browse/HDDS-12301). Move `ozone.om.volume.listall.allowed` into `OmConfig`
- [HDDS-12248](https://issues.apache.org/jira/browse/HDDS-12248). Make `allowListAllVolumes` reconfigurable in OM
- [HDDS-14207](https://issues.apache.org/jira/browse/HDDS-14207). Inconsistent Ozone admin check (volume listall check)
