---
slug: /component/om/om-multitenant
sidebar_label: "om-multitenant"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/om/om-multitenant.md`. Author prose belongs above or below the BEGIN/END markers._
...
click MultiTenantAccessController href "/class/multitenantaccesscontroller" "Open MultiTenantAccessController"
click InMemoryMultiTenantAccessController href "/class/inmemorymultitenantaccesscontroller" "Open InMemoryMultiTenantAccessController"
click AuthorizerLock href "/class/authorizerlock" "Open AuthorizerLock"
click AuthorizerLockImpl href "/class/authorizerlockimpl" "Open AuthorizerLockImpl"
click CachedTenantState href "/class/cachedtenantstate" "Open CachedTenantState"
```
...
|--:|---|---|---|--:|--:|---|
| 1431 | <Fqcn>org.apache.hadoop.ozone.om.multitenant.MultiTenantAccessController</Fqcn> | interface | logic-heavy | 375~ | 20 | Defines the operations needed for multi-tenant access control. |
| 1432 | <Fqcn>org.apache.hadoop.ozone.om.multitenant.AuthorizerLock</Fqcn> | interface | mixed | 25~ | 20 | Authorizer access lock interface. |
| 1433 | <Fqcn>org.apache.hadoop.ozone.om.multitenant.InMemoryMultiTenantAccessController</Fqcn> | service | mixed | 125~ | 30 | In Memory version of MultiTenantAccessController. |
| 1434 | <Fqcn>org.apache.hadoop.ozone.om.multitenant.AuthorizerLockImpl</Fqcn> | service | mixed | 125~ | 30 | Implementation of AuthorizerLock. |
| 1435 | <Fqcn>org.apache.hadoop.ozone.om.multitenant.CachedTenantState</Fqcn> | service | mixed | 75~ | 30 | A collection of things that we want to maintain about a tenant in memory. |
...
The interface defines `createTenant`, `deleteTenant`, `createRole`, `deleteRole`, `createAccessPolicy`, `deleteAccessPolicy`, `assignUserToRole`, `revokeUserFromRole`, etc. The `build(conf, authorizer)` static factory selects the implementation: if `authorizer` is a Ranger-backed implementation, it returns a `RangerClient`-based controller (from the `ozone-multitenant-ranger` module). For tests and secure-mode-off, `InMemoryMultiTenantAccessController` is used. This was introduced in [HDDS-12451](https://issues.apache.org/jira/browse/HDDS-12451) as a factory for the multi-tenant access controller.
...
- [HDDS-12451](https://issues.apache.org/jira/browse/HDDS-12451). Create factory for MultiTenantAccessController
- [HDDS-12454](https://issues.apache.org/jira/browse/HDDS-12454). Create new module for multitenancy with Ranger
- [HDDS-7178](https://issues.apache.org/jira/browse/HDDS-7178). Use optimistic read in Ranger background sync (AuthorizerLock)
- [HDDS-11587](https://issues.apache.org/jira/browse/HDDS-11587). Ozone Manager not processing file put requests with multi-tenancy enabled
- [HDDS-7042](https://issues.apache.org/jira/browse/HDDS-7042). Rebuilding tenant cache omits empty tenants
...
Answer 1: `tryReadLock(timeout)` throws `OMException` with a timeout error if the lock cannot be acquired within the specified duration ([HDDS-6990](https://issues.apache.org/jira/browse/HDDS-6990) clarified the semantics).
