---
slug: /component/hddscommon/storage-common
sidebar_label: "storage-common"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/hddscommon/storage-common.md`. Author prose belongs above or below the BEGIN/END markers._
...
|--:|---|---|---|--:|--:|---|
| 2070 | <Fqcn>org.apache.hadoop.hdds.scm.storage.ContainerProtocolCalls</Fqcn> | service | logic-heavy | 700~ | 60 | Implementation of all container protocol calls performed by Container clients. |
| 2071 | <Fqcn>org.apache.hadoop.hdds.scm.storage.BlockLocationInfo</Fqcn> | dto | data-only | 150~ | 10 | One key can be too huge to fit in one container. |
...
- **note:** Every public method is a static factory for a specific container command; the caller supplies the `XceiverClientSpi` and bears thread-safety responsibility. The streaming `readBlock` overload ([HDDS-13973](https://issues.apache.org/jira/browse/HDDS-13973)/13974/13975) reuses the same gRPC stream for all chunks in one block, avoiding repeated stream setup overhead at the cost of requiring the caller to drive back-pressure on the response iterator.
...
- [HDDS-13973](https://issues.apache.org/jira/browse/HDDS-13973). The ground work to support stream read block
- [HDDS-13974](https://issues.apache.org/jira/browse/HDDS-13974). Use the same gRPC stream for reading the same block
- [HDDS-13975](https://issues.apache.org/jira/browse/HDDS-13975). Limit the number of responses in stream read block
- [HDDS-13680](https://issues.apache.org/jira/browse/HDDS-13680). migrate tracing to opentelemetry
- [HDDS-15791](https://issues.apache.org/jira/browse/HDDS-15791). EC reconstructed RECOVERING container can be deleted by SCM
...
- The streaming `readBlock` path ([HDDS-13973](https://issues.apache.org/jira/browse/HDDS-13973)) accumulates response count via a semaphore limit; exceeding `ozone.scm.block.stream.read.max.inflight` silently stalls rather than throwing, making tuning mistakes hard to diagnose.

## Related features

- [pipeline-common.md](/component/hddscommon/pipeline-common) — `Pipeline` is the routing object carried inside `BlockLocationInfo`
- [container-common.md](/component/hddscommon/container-common) — container-layer DTOs consumed by `ContainerProtocolCalls`
- [security-tokens.md](/component/hddscommon/security-tokens) — `OzoneBlockTokenIdentifier` attached to `BlockLocationInfo`
- [tracing-common.md](/component/hddscommon/tracing-common) — `TracingUtil` used by `ContainerProtocolCalls` for span creation
- [scm-client-proxy.md](/component/hddscommon/scm-client-proxy) — `XceiverClientSpi` implementations that `ContainerProtocolCalls` delegates to
...
Answer 1: [HDDS-13973](https://issues.apache.org/jira/browse/HDDS-13973) introduced the streaming read variant. It enabled pipelined chunk transfers over a single gRPC stream, avoiding repeated stream setup for each chunk of a block.
