---
slug: /component/hddscommon/lease-manager
sidebar_label: "lease-manager"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/hddscommon/lease-manager.md`. Author prose belongs above or below the BEGIN/END markers._
...
|--:|---|---|---|--:|--:|---|
| 1952 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseManager</Fqcn> | service | mixed | 150~ | 45 | LeaseManager is someone who can provide you leases based on your requirement. |
| 1953 | <Fqcn>org.apache.hadoop.ozone.lease.Lease</Fqcn> | service | mixed | 75~ | 30 | This class represents the lease created on a resource. |
| 1954 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseCallbackExecutor</Fqcn> | service | mixed | 25~ | 30 | This class is responsible for executing the callbacks of a lease in case of timeout. |
| 1955 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseExpiredException</Fqcn> | exception | data-only | 25~ | 10 | This exception represents that the lease that is being accessed has expired. |
| 1956 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseManagerNotRunningException</Fqcn> | exception | data-only | 25~ | 10 | This exception represents that there LeaseManager service is not running. |
| 1957 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseException</Fqcn> | exception | data-only | 25~ | 10 | This exception represents all lease related exceptions. |
| 1958 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseAlreadyExistException</Fqcn> | exception | data-only | 25~ | 10 | This exception represents that there is already a lease acquired on the same resource. |
| 1959 | <Fqcn>org.apache.hadoop.ozone.lease.LeaseNotFoundException</Fqcn> | exception | data-only | 25~ | 10 | This exception represents that the lease that is being accessed does not exist. |
...
- [HDDS-12641](https://issues.apache.org/jira/browse/HDDS-12641). Move Lease to hdds-server-framework
- [HDDS-12897](https://issues.apache.org/jira/browse/HDDS-12897). Enable EmptyLineSeparator checkstyle rule
...
- [`framework-server.md`](/component/hddscommon/framework-server) — server lifecycle utilities in the same `hdds-server-framework` module where `LeaseManager` now lives after [HDDS-12641](https://issues.apache.org/jira/browse/HDDS-12641)
- [`pipeline-common.md`](/component/hddscommon/pipeline-common) — SCM pipeline provisioning that uses `LeaseManager` for pipeline creation timeouts
- [`scm-common.md`](/component/hddscommon/scm-common) — SCM-side resource management that coordinates with lease-based timeouts
- [`ratis-integration.md`](/component/hddscommon/ratis-integration) — Ratis pipeline setup whose SCM-side lifecycle interacts with lease expiry
