---
slug: /component/hddscommon/framework-freon
sidebar_label: "framework-freon"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/hddscommon/framework-freon.md`. Author prose belongs above or below the BEGIN/END markers._
...
click FakeClusterTopology href "/class/fakeclustertopology" "Open FakeClusterTopology"
click FakeScmBlockLocationProtocolClient href "/class/fakescmblocklocationprotocolclient" "Open FakeScmBlockLocationProtocolClient"
click FakeScmContainerLocationProtocolClient href "/class/fakescmcontainerlocationprotocolclient" "Open FakeScmContainerLocationProtocolClient"
...
|--:|---|---|---|--:|--:|---|
| 1676 | <Fqcn>org.apache.hadoop.hdds.freon.FakeClusterTopology</Fqcn> | service | mixed | 50~ | 30 | inferred: FakeClusterTopology — role not documented. |
| 1677 | <Fqcn>org.apache.hadoop.hdds.freon.FakeScmBlockLocationProtocolClient</Fqcn> | service | mixed | 50~ | 30 | Fake SCM client to return a simulated block location. |
| 1678 | <Fqcn>org.apache.hadoop.hdds.freon.FakeScmContainerLocationProtocolClient</Fqcn> | service | mixed | 25~ | 30 | Fake SCM client to return a simulated block location. |
...
- [HDDS-3621](https://issues.apache.org/jira/browse/HDDS-3621). Separate client/server/admin proto files of HDDS to separated subprojects
...
- [`pipeline-common.md`](/component/hddscommon/pipeline-common) — `Pipeline` and `PipelineID` are the core types that `FakeClusterTopology` generates and the fake clients return.
- [`container-common.md`](/component/hddscommon/container-common) — `ContainerWithPipeline` and `AllocatedBlock` are the response types the fake clients produce.
- [`protocol-common.md`](/component/hddscommon/protocol-common) — `ScmBlockLocationProtocol` and `ScmContainerLocationProtocol` interfaces that the fake clients implement.
- [`framework-protocol.md`](/component/hddscommon/framework-protocol) — real SCM protocol client implementations that the fake clients shadow for load-testing.
...
5. [HDDS-3621](https://issues.apache.org/jira/browse/HDDS-3621) separated client/server/admin proto files. How does that split affect where `FakeScmBlockLocationProtocolClient` must reside in the module hierarchy?
...
Answer 5: After [HDDS-3621](https://issues.apache.org/jira/browse/HDDS-3621), the client-facing SCM protocols live in a dedicated interface submodule; `FakeScmBlockLocationProtocolClient` must depend on that module. Placing it in `hadoop-hdds/framework` (which already depends on the interface modules) satisfies this without adding a new module dependency.
