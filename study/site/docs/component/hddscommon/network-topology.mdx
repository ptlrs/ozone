---
slug: /component/hddscommon/network-topology
...
_This feature page was generated from `study/atlas/components/hddscommon/network-topology.md`. Author prose belongs above or below the BEGIN/END markers._
...
click NetworkTopology href "/class/networktopology" "Open NetworkTopology"
click NetworkTopologyImpl href "/class/networktopologyimpl" "Open NetworkTopologyImpl"
click InnerNode href "/class/innernode" "Open InnerNode"
click InnerNodeImpl href "/class/innernodeimpl" "Open InnerNodeImpl"
click Node href "/class/node" "Open Node"
click NodeImpl href "/class/nodeimpl" "Open NodeImpl"
click NodeSchemaLoader href "/class/nodeschemaloader" "Open NodeSchemaLoader"
click NodeSchemaManager href "/class/nodeschemamanager" "Open NodeSchemaManager"
click NodeSchema href "/class/nodeschema" "Open NodeSchema"
click NetUtils href "/class/netutils" "Open NetUtils"
click NetConstants href "/class/netconstants" "Open NetConstants"
click HostAndPort href "/class/hostandport" "Open HostAndPort"
```
...
|--:|---|---|---|--:|--:|---|
| 1964 | <Fqcn>org.apache.hadoop.hdds.scm.net.NodeImpl</Fqcn> | service | mixed | 175~ | 20 | A thread safe class that implements interface Node. |
| 1965 | <Fqcn>org.apache.hadoop.hdds.scm.net.Node</Fqcn> | interface | mixed | 25~ | 20 | The interface defines a node in a network topology. |
| 1966 | <Fqcn>org.apache.hadoop.hdds.scm.net.NetworkTopology</Fqcn> | interface | mixed | 25~ | 20 | The interface defines a network topology. |
| 1967 | <Fqcn>org.apache.hadoop.hdds.scm.net.InnerNode</Fqcn> | interface | mixed | 25~ | 20 | The interface defines an inner node in a network topology. |
| 1968 | <Fqcn>org.apache.hadoop.hdds.scm.net.NetworkTopologyImpl</Fqcn> | service | logic-heavy | 600~ | 60 | The class represents a cluster of computers with a tree hierarchical network topology. |
| 1969 | <Fqcn>org.apache.hadoop.hdds.scm.net.InnerNodeImpl</Fqcn> | service | logic-heavy | 425~ | 60 | A thread safe class that implements InnerNode interface. |
| 1970 | <Fqcn>org.apache.hadoop.hdds.scm.net.NodeSchemaLoader</Fqcn> | service | logic-heavy | 350~ | 45 | A Network topology layer schema loading tool that loads user defined network layer schema data from a XML configurati... |
| 1971 | <Fqcn>org.apache.hadoop.hdds.scm.net.NodeSchemaManager</Fqcn> | service | mixed | 100~ | 30 | The class manages all network topology schemas. |
| 1972 | <Fqcn>org.apache.hadoop.hdds.scm.net.NetUtils</Fqcn> | service | mixed | 75~ | 30 | Utility class to facilitate network topology functions. |
| 1973 | <Fqcn>org.apache.hadoop.hdds.scm.net.HostAndPort</Fqcn> | service | mixed | 50~ | 30 | A class for host and port. |
| 1974 | <Fqcn>org.apache.hadoop.hdds.scm.net.NetConstants</Fqcn> | service | mixed | 25~ | 30 | Class to hold network topology related constants and configurations. |
| 1975 | <Fqcn>org.apache.hadoop.hdds.scm.net.NodeSchema</Fqcn> | service | mixed | 100~ | 10 | Network topology schema to housekeeper relevant information. |
...
- [HDDS-14934](https://issues.apache.org/jira/browse/HDDS-14934). Enable PMD rule ConsecutiveAppendsShouldReuse
- [HDDS-14857](https://issues.apache.org/jira/browse/HDDS-14857). Use XMLUtils.newSecureFactory
- [HDDS-14609](https://issues.apache.org/jira/browse/HDDS-14609). Remove dependency on snakeyaml from hdds-common

## Sharp edges

- [HDDS-14857](https://issues.apache.org/jira/browse/HDDS-14857): before the fix, `NodeSchemaLoader`'s XML fallback path used a default `DocumentBuilderFactory` without disabling external entity resolution, making SCM vulnerable to XXE if a malicious topology file was placed at the configured path; the fix is in `NodeSchemaLoader.java` in the XML parsing block.
- [HDDS-14609](https://issues.apache.org/jira/browse/HDDS-14609): snakeyaml was replaced with a secure alternative in `NodeSchemaLoader`; if your branch depends on snakeyaml types being on the hdds-common classpath for topology parsing, they will be absent after this change.

## Related features

- [`scm-common.md`](/component/hddscommon/scm-common) — SCM pipeline and container placement that consumes `NetworkTopologyImpl.chooseRandom()`
- [`pipeline-common.md`](/component/hddscommon/pipeline-common) — pipeline creation logic that uses rack-aware node selection
- [`protocol-common.md`](/component/hddscommon/protocol-common) — `DatanodeDetails` carries the network location string that `NodeImpl` stores
- [`config-runtime.md`](/component/hddscommon/config-runtime) — `ConfigurationSource` passed to `NetworkTopologyImpl` and `NodeSchemaManager.init()`
- [`framework-utils.md`](/component/hddscommon/framework-utils) — `YamlUtils` used by `NodeSchemaLoader` for YAML parsing
