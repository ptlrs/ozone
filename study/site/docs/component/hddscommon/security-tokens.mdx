---
slug: /component/hddscommon/security-tokens
...
_This feature page was generated from `study/atlas/components/hddscommon/security-tokens.md`. Author prose belongs above or below the BEGIN/END markers._
...
|--:|---|---|---|--:|--:|---|
| 2067 | <Fqcn>org.apache.hadoop.hdds.security.token.ShortLivedTokenIdentifier</Fqcn> | abstract | mixed | 75~ | 30 | Base class for short-lived tokens (block, container). |
| 2068 | <Fqcn>org.apache.hadoop.hdds.security.token.OzoneBlockTokenIdentifier</Fqcn> | service | mixed | 125~ | 30 | Block token identifier for Ozone/HDDS. |
| 2069 | <Fqcn>org.apache.hadoop.hdds.security.token.OzoneBlockTokenSelector</Fqcn> | service | mixed | 25~ | 30 | A block token selector for Ozone. |
...
- `OzoneBlockTokenIdentifier` carries a `maxLength` field that the datanode checks to reject writes beyond the allocation; if OM issues a token with an incorrect `maxLength` (e.g., after a partial write is resumed), the datanode will reject the continuation write with a token validation error ([HDDS-10054](https://issues.apache.org/jira/browse/HDDS-10054) context).
...
- [`security-common.md`](/component/hddscommon/security-common) — `SecurityConfig` and key management that back the HMAC secret
- [`scm-client-proxy.md`](/component/hddscommon/scm-client-proxy) — `SCMSecurityProtocolFailoverProxyProvider` used to fetch secret key renewals from SCM
- [`container-common.md`](/component/hddscommon/container-common) — container-level RPC that carries the block token
- [`protocol-common.md`](/component/hddscommon/protocol-common) — gRPC stubs through which the token travels to the datanode
...
3. [HDDS-7831](https://issues.apache.org/jira/browse/HDDS-7831) switched block token signing from RSA to HMAC-SHA256. What is the concrete performance benefit on the datanode, and what security assumption does HMAC require that RSA does not?
