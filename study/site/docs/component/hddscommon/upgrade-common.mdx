---
slug: /component/hddscommon/upgrade-common
sidebar_label: "upgrade-common"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/hddscommon/upgrade-common.md`. Author prose belongs above or below the BEGIN/END markers._
...
click LayoutFeature href "/class/layoutfeature" "Open LayoutFeature"
click HDDSLayoutFeature href "/class/hddslayoutfeature" "Open HDDSLayoutFeature"
click HDDSUpgradeAction href "/class/hddsupgradeaction" "Open HDDSUpgradeAction"
click BelongsToHDDSLayoutVersion href "/class/belongstohddslayoutversion" "Open BelongsToHDDSLayoutVersion"
click UpgradeFinalization href "/class/upgradefinalization" "Open UpgradeFinalization"
click UpgradeException href "/class/upgradeexception" "Open UpgradeException"
```
...
|--:|---|---|---|--:|--:|---|
| 2080 | <Fqcn>org.apache.hadoop.hdds.upgrade.BelongsToHDDSLayoutVersion</Fqcn> | interface | mixed | 25~ | 20 | Annotation to mark a class or a field declaration that belongs to a specific HDDS Layout Version. |
| 2081 | <Fqcn>org.apache.hadoop.hdds.upgrade.HDDSUpgradeAction</Fqcn> | interface | mixed | 25~ | 20 | Upgrade Action for SCM and DataNodes. |
| 2082 | <Fqcn>org.apache.hadoop.hdds.upgrade.HDDSLayoutFeature</Fqcn> | data | data-only | 50~ | 10 | List of HDDS Features. |

### Sub-feature: `ozone.upgrade`

| reading_order | fqcn | kind | logic | loc | study (min) | role |
|--:|---|---|---|--:|--:|---|
| 2083 | <Fqcn>org.apache.hadoop.ozone.upgrade.LayoutFeature</Fqcn> | interface | mixed | 25~ | 20 | Generic Layout feature interface for Ozone. |
| 2084 | <Fqcn>org.apache.hadoop.ozone.upgrade.UpgradeFinalization</Fqcn> | service | mixed | 100~ | 10 | Client-side interface of upgrade finalization. |
| 2085 | <Fqcn>org.apache.hadoop.ozone.upgrade.UpgradeException</Fqcn> | exception | data-only | 25~ | 10 | Exception thrown when upgrade fails. |
...
- [HDDS-11963](https://issues.apache.org/jira/browse/HDDS-11963). Add parent interface of component and layout versions for use in request validator
...
- [HDDS-12420](https://issues.apache.org/jira/browse/HDDS-12420). Move FinalizeUpgradeCommandUtil to hdds-common
...
- [HDDS-14569](https://issues.apache.org/jira/browse/HDDS-14569) removed the `BEFORE_FINALIZATION` action phase; any `HDDSUpgradeAction` implementation that still references the old `RunWith` enum variant will fail to compile against current code, a trap for backports.

## Related features

- [upgrade-framework.md](/component/hddscommon/upgrade-framework) — `BasicUpgradeFinalizer` and `AbstractLayoutVersionManager` implement these interfaces
- [framework-server.md](/component/hddscommon/framework-server) — service startup registers `HDDSLayoutVersionManager` which manages `HDDSLayoutFeature` constants
- [scm-common.md](/component/hddscommon/scm-common) — SCM-side layout feature checks use `LayoutFeature` gating
- [ozone-common-primitives.md](/component/hddscommon/ozone-common-primitives) — `Versioned` interface that `LayoutFeature` extends

## Self-quiz

1. Why must new `HDDSLayoutFeature` enum constants always be appended rather than inserted at an earlier position?
2. What was removed from the upgrade model in [HDDS-14569](https://issues.apache.org/jira/browse/HDDS-14569), and why was that simplification safe?
