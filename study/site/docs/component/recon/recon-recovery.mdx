---
slug: /component/recon/recon-recovery
sidebar_label: "recon-recovery"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/recon/recon-recovery.md`. Author prose belongs above or below the BEGIN/END markers._
...
click ReconOMMetadataManager href "/class/reconommetadatamanager" "Open ReconOMMetadataManager"
click ReconOmMetadataManagerImpl href "/class/reconommetadatamanagerimpl" "Open ReconOmMetadataManagerImpl"
...
|--:|---|---|---|--:|--:|---|
| 2378 | <Fqcn>org.apache.hadoop.ozone.recon.recovery.ReconOMMetadataManager</Fqcn> | interface | mixed | 25~ | 20 | Interface for the OM Metadata Manager + DB store maintained by Recon. |
| 2379 | <Fqcn>org.apache.hadoop.ozone.recon.recovery.ReconOmMetadataManagerImpl</Fqcn> | service | logic-heavy | 200~ | 45 | Recon's implementation of the OM Metadata manager. |
...
- [HDDS-11349](https://issues.apache.org/jira/browse/HDDS-11349). Add NullPointer handling when volume/bucket tables are not initialized
- [HDDS-11484](https://issues.apache.org/jira/browse/HDDS-11484). Validate javadoc in CI
- [HDDS-11660](https://issues.apache.org/jira/browse/HDDS-11660). Recon List Key API: Reduce object creation and buffering memory
- [HDDS-13576](https://issues.apache.org/jira/browse/HDDS-13576). Recon reprocess of all tasks should be non-blocking
...
- `updateOmDB()` closes and reopens the entire `DBStore`. If any thread is mid-read on a table handle at that moment, it will see a closed-store exception. The `omTablesInitialized` guard only prevents new reads from starting; it does not protect in-flight reads. ([HDDS-15863](https://issues.apache.org/jira/browse/HDDS-15863) added a manual rebuild path that worsens this window.)
...
5. [HDDS-15863](https://issues.apache.org/jira/browse/HDDS-15863) added a manual OM DB rebuild path. What scenario motivated this and how does `updateOmDB()` support it?
...
Answer 5: In some disaster-recovery or corruption scenarios, Recon's local OM snapshot becomes too far behind or corrupt and delta-sync cannot recover it. [HDDS-15863](https://issues.apache.org/jira/browse/HDDS-15863) added a `/dbsync/reset` admin endpoint that triggers a fresh full snapshot download and calls `updateOmDB()` with the new checkpoint, bypassing the normal delta-sync interval.
