---
slug: /component/recon/recon-heatmap
sidebar_label: "recon-heatmap"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/recon/recon-heatmap.md`. Author prose belongs above or below the BEGIN/END markers._
...
click IHeatMapProvider href "/class/iheatmapprovider" "Open IHeatMapProvider"
click HeatMapService href "/class/heatmapservice" "Open HeatMapService"
click HeatMapServiceImpl href "/class/heatmapserviceimpl" "Open HeatMapServiceImpl"
click HeatMapUtil href "/class/heatmaputil" "Open HeatMapUtil"
```
...
|--:|---|---|---|--:|--:|---|
| 2358 | <Fqcn>org.apache.hadoop.ozone.recon.heatmap.IHeatMapProvider</Fqcn> | interface | mixed | 25~ | 20 | This interface is to provide heatmap data. |
| 2359 | <Fqcn>org.apache.hadoop.ozone.recon.heatmap.HeatMapServiceImpl</Fqcn> | service | mixed | 75~ | 30 | This class is an implementation of abstract class for retrieving data through HeatMapService. |
| 2360 | <Fqcn>org.apache.hadoop.ozone.recon.heatmap.HeatMapService</Fqcn> | abstract | mixed | 25~ | 30 | This is an abstract class for implementation of access to HeatMap Service. |
| 2361 | <Fqcn>org.apache.hadoop.ozone.recon.heatmap.HeatMapUtil</Fqcn> | service | logic-heavy | 300~ | 45 | This class is general utility class for keeping heatmap utility functions. |
...
- [HDDS-7795](https://issues.apache.org/jira/browse/HDDS-7795). Recon - Data Management Metrics - HeatMap of Ozone Data (initial feature)
- [HDDS-8414](https://issues.apache.org/jira/browse/HDDS-8414). Recon: Heatmap Absolute Path in Response at end node level
- [HDDS-8743](https://issues.apache.org/jira/browse/HDDS-8743). Recon - Expose API for hide/show flag for heatmap feature
- [HDDS-8852](https://issues.apache.org/jira/browse/HDDS-8852). Ozone Recon Heatmap API - 500 server error on entityType as volume
- [HDDS-9087](https://issues.apache.org/jira/browse/HDDS-9087). Ozone Recon Heatmap code refactoring and Bucket level access count fix
- [HDDS-11228](https://issues.apache.org/jira/browse/HDDS-11228). Ozone Recon HeatMap refactoring of code
...
- `HeatMapUtil` recursively traverses the namespace tree but does not impose a strict node-count cap per request; on very large volumes with many sub-directories the response can be very large. ([HDDS-10452](https://issues.apache.org/jira/browse/HDDS-10452) added Top-N limiting to disk-usage but heatmap recursion depth is still bounded loosely.)
...
5. After [HDDS-9087](https://issues.apache.org/jira/browse/HDDS-9087), how does `HeatMapUtil` handle bucket-level access count aggregation differently from the original implementation?
...
Answer 5: After [HDDS-9087](https://issues.apache.org/jira/browse/HDDS-9087), bucket-level aggregation rolls up access counts from child keys rather than reporting zero when no direct bucket-level access record exists, fixing incorrect zeros in the heatmap for buckets accessed only through their keys.
