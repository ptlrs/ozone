---
slug: /component/dn/dn-helpers
sidebar_label: "dn-helpers"
atlasGenerated: true
---
...
_This feature page was generated from `study/atlas/components/dn/dn-helpers.md`. Author prose belongs above or below the BEGIN/END markers._
...
click ContainerUtils href "/class/containerutils" "Open ContainerUtils"
click DatanodeIdYaml href "/class/datanodeidyaml" "Open DatanodeIdYaml"
click TokenHelper href "/class/tokenhelper" "Open TokenHelper"
click ContainerMetrics href "/class/containermetrics" "Open ContainerMetrics"
click BlockDeletingServiceMetrics href "/class/blockdeletingservicemetrics" "Open BlockDeletingServiceMetrics"
click CommandHandlerMetrics href "/class/commandhandlermetrics" "Open CommandHandlerMetrics"
```
...
|--:|---|---|---|--:|--:|---|
| 1527 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.ContainerUtils</Fqcn> | service | logic-heavy | 275~ | 45 | A mix of helper functions for containers. |
| 1528 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.DatanodeIdYaml</Fqcn> | service | logic-heavy | 200~ | 45 | Class for creating datanode.id file in yaml format. |
| 1529 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.DeletedContainerBlocksSummary</Fqcn> | service | mixed | 75~ | 30 | A helper class to wrap the info about under deletion container blocks. |
| 1530 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.TokenHelper</Fqcn> | service | mixed | 50~ | 30 | Wraps block and container token managers for datanode. |
| 1531 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.DatanodeVersionFile</Fqcn> | service | mixed | 25~ | 30 | This is a utility class which helps to create the version file on datanode and also validate the content of the versi... |
| 1532 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.ContainerMetrics</Fqcn> | metrics | mixed | 150~ | 20 | This class is for maintaining  the various Storage Container DataNode statistics and publishing them through the metr... |
| 1533 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.BlockDeletingServiceMetrics</Fqcn> | metrics | mixed | 150~ | 20 | Metrics related to Block Deleting Service running on Datanode. |
| 1534 | <Fqcn>org.apache.hadoop.ozone.container.common.helpers.CommandHandlerMetrics</Fqcn> | metrics | mixed | 75~ | 20 | This class collects and exposes metrics for CommandHandlerMetrics. |
...
Contains `verifyChecksum(data, checksum)` used by chunk readers and `getContainerDBPath(volume, id)` used when opening per-container RocksDB instances. The method `parseChecksum(bytes, checksumType)` feeds into the container scanner's data scan. [HDDS-12151](https://issues.apache.org/jira/browse/HDDS-12151) added a minimum free-space guard here that `HddsDispatcher` calls before accepting a write.
...
- [HDDS-13955](https://issues.apache.org/jira/browse/HDDS-13955). Handle empty `datanode.id` file gracefully.
- [HDDS-12151](https://issues.apache.org/jira/browse/HDDS-12151). Fail write when volume is full, considering min free space.
- [HDDS-12531](https://issues.apache.org/jira/browse/HDDS-12531). Use `AtomicFileOutputStream` to write YAML files.
...
- [HDDS-1480](https://issues.apache.org/jira/browse/HDDS-1480). Prefer resolved datanode IP address over persisted IP address.

## Sharp edges

- `DatanodeIdYaml.readDatanodeIdFile` returns `null` if the file is empty ([HDDS-13955](https://issues.apache.org/jira/browse/HDDS-13955)); callers that do not null-check will NPE at registration time, causing the datanode to fail to start instead of generating a new identity.
